...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [profileRes, todoRes, leaderboardRes] = await Promise.all([
          api.get('/api/profile'),
          api.get('/api/user/todo'),
          api.get('/api/leaderboard')
        ]);
        console.log('Todo List Response:', todoRes.data);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-8 bg-[#112240] rounded-lg p-6 shadow-lg border border-[#1e2a47] hover:border-[#64ffda] transition-all">
          <div className="flex items-center mb-4">
            ...
            <h3 className="text-xl font-semibold">Agent Leaderboard</h3>
          </div>
                ...
                <tbody className="divide-y divide-[#1e2a47]">
                  {leaderboard.map((player, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-[#ccd6f6]">{player.points}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-[#ccd6f6]">{player.solvedChallenges?.length || 0}</td>
                      <td className="px-4 py-3 whitespace-nowrap text-[#ccd6f6]">
                        Level {player.points ? Math.min(10, Math.floor(player.points/100) + 1) : 1}
                      </td>
                    </tr>
                  ))}
                </tbody>
